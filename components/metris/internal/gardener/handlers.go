...
func (c *Controller) secretUpdateHandlerFunc(oldObj, newObj interface{}) {
	...
	if len(shootlist.Items) == 0 {
		c.logger.Debugf("no shoots found with secret binding '%s'", newSecret.Name)
		return
	}

	for i := range shootlist.Items {
		shoot := shootlist.Items[i]
		c.logger.Debugf("secret '%s' has been modified, updating cluster config for '%s'", newSecret.Name, shoot.Name)
...
func (c *Controller) shootDeleteHandlerFunc(obj interface{}) {
	...
	logger := c.logger.With("account", cluster.AccountID).With("subaccount", cluster.SubAccountID)
	...
	logger.With("shoot", cluster.TechnicalID).Infof("shoot is being deleted")
...
func (c *Controller) shootAddHandlerFunc(obj interface{}) {
	...
	logger := c.logger.With("account", cluster.AccountID).With("subaccount", cluster.SubAccountID).With("shoot", cluster.TechnicalID)
	...
	logger.Debug("sending cluster information to provider handler")
...
func (c *Controller) shootUpdateHandlerFunc(oldObj, newObj interface{}) {
	...
	if err != nil {
		c.logger.With("account", c1.AccountID).With("subaccount", c1.SubAccountID).With("shoot", c1.TechnicalID).Error(err)
		return
	}

	c2, err := c.newCluster(newShoot)
	if err != nil {
		c.logger.With("account", c2.AccountID).With("subaccount", c2.SubAccountID).With("shoot", c2.TechnicalID).Error(err)
		return
	}
	...
	c.logger.With("account", c2.AccountID).With("subaccount", c2.SubAccountID).With("shoot", c2.TechnicalID).Warn("shoot has been modified")
...
func (c *Controller) newCluster(shoot *gcorev1beta1.Shoot) (*Cluster, error) {
	...
	if err != nil {
		err = fmt.Errorf("could not get technical id for shoot '%s'", shoot.Name)

		metrics.ClusterSyncFailureVec.WithLabelValues("technicalid").Inc()
	...
	if !ok || cluster.AccountID == "" {
		...
		metrics.ClusterSyncFailureVec.WithLabelValues("accountid").Inc()
	...
	if !ok || cluster.SubAccountID == "" {
		...
		metrics.ClusterSyncFailureVec.WithLabelValues("subaccountid").Inc()
	...
	if err != nil {
		...
		metrics.ClusterSyncFailureVec.WithLabelValues("secret").Inc()
