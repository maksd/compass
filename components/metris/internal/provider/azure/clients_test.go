...
import (
	...
	"github.com/kyma-incubator/compass/components/metris/internal/provider/azure/mocks"
...
func newTestServer(responses map[string]interface{}) *httptest.Server {
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// have to remove timestamp from url because value keep changing
		re := regexp.MustCompile(`(\?|&)timespan=[^&]*`)
...
func Test_clients(t *testing.T) {
	...
	s := newTestServer(map[string]interface{}{
		urlEscape("%s/resourcegroups?$filter=%s&api-version=2019-05-01", baseuri, rgfilter):                    grouplist,
		urlEscape("%s/resourcegroups/test-resourcegroup?api-version=2019-05-01", baseuri):                      group,
		urlEscape("%s/providers/Microsoft.Compute/virtualMachines?api-version=2019-12-01", rgbaseuri):          vmlist,
		urlEscape("%s/providers/Microsoft.Compute/disks?api-version=2019-11-01", rgbaseuri):                    disklist,
		urlEscape("%s/providers/Microsoft.Network/loadBalancers?api-version=2020-05-01", rgbaseuri):            lblist,
		urlEscape("%s/providers/Microsoft.Network/virtualNetworks?api-version=2020-05-01", rgbaseuri):          netlist,
		urlEscape("%s/providers/Microsoft.Network/publicIPAddresses?api-version=2020-05-01", rgbaseuri):        iplist,
		urlEscape("%s/providers/Microsoft.EventHub/namespaces?api-version=2017-04-01", ehrgbaseuri):            nslist,
		urlEscape("%s/providers/Microsoft.Compute/skus?$filter=%s&api-version=2019-04-01", baseuri, skufilter): skulist,
	})
	defer s.Close()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.ctype {
			case "ResourceGroup":
				obj, err = client.GetResourceGroup(tt.rgname, tt.filter, noopLogger)
			...
			case "VirtualMachine":
				obj, err = client.GetVirtualMachines(context.Background(), tt.rgname)
...
func Test_client_getMetricValues(t *testing.T) {
	...
	params1 := "aggregation=Maximum&api-version=2018-01-01&interval=PT5M&metricnames=IncomingBytes,OutgoingBytes,IncomingMessages&resultType=Data"
	novalueparams := "aggregation=Maximum&api-version=2018-01-01&interval=PT5M&metricnames=NoValueMetric&resultType=Data"
	notsparams := "aggregation=Maximum&api-version=2018-01-01&interval=PT5M&metricnames=NoTSMetric&resultType=Data"
	notsdataparams := "aggregation=Maximum&api-version=2018-01-01&interval=PT5M&metricnames=NoTSDataMetric&resultType=Data"
