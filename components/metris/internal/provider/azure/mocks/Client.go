...
// GetResourceGroup provides a mock function with given fields: name, filter, logger
func (_m *Client) GetResourceGroup(name string, filter string, logger *zap.SugaredLogger) (resources.Group, error) {
	ret := _m.Called(name, filter, logger)

	var r0 resources.Group
	if rf, ok := ret.Get(0).(func(string, string, *zap.SugaredLogger) resources.Group); ok {
		r0 = rf(name, filter, logger)
	} else {
		r0 = ret.Get(0).(resources.Group)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string, string, *zap.SugaredLogger) error); ok {
		r1 = rf(name, filter, logger)
	} else {
		r1 = ret.Error(1)
	}
